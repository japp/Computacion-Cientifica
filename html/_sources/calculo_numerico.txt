...
Aunque el paquete :mod:`Numpy` ofrece ciertas funcionalidades matemáticas además de la manipulación básica de arrays, como el paquete :mod:`linalg` para álgebra lineal y :mod:`random` para números aleatorios, en :mod:`Scipy` encontraremos todas las herramientas matemáticas que podamos necesitar. :mod:`Scipy` es una colección de paquetes de algoritmos y herramientas matemáticas para distintas tareas, que también utiliza :mod:`Numpy`. :mod:`Scipy` posee varios subpaquetes que deben importarse independientemente cuando se vayan a utilizar; éstos son algunos de ellos:
...
Para ver la lista completa de subpaquetes consultar la ayuda de scipy como :func:`help(scipy)` (haciendo antes ``import scipy`` para tener todos los nombres asociados a :mod:`scipy`) y consultar su página web para ver la documentación completa (`Link www.scipy.org`). Una manera práctica de trabajar es importar el espacio de nombres de :mod:`scipy`, es decir el nombre de sus paquetes y funciones principales y luego importar el o los paquetes que vaya a usar, como en este ejemplo::
...
El subpaquete :mod:`integrate` ofrece varias herramientas de integración numérica con distintos métodos. Podemos ver todos los disponibles consultando su ayuda::
...
Existen por ejemplo implementaciones del método del trapecio o el método de Simpsom, ambos métodos simples basados en muestras fijas. Las funciones :func:`trapz()` y :func:`simps()`, que emplean dichos métodos para al cálculo numérico de integrales, adminten como entrada un array ``y`` de valores a integrar y otro array con la variable independiente ``x``; si no se incluye un segundo parámetro, el espaciado entre los elementos de ``y`` de 1 por defecto, aunque este valor se puede asignar con el parámetro opcional ``dx=1``. 

Supongamos que queremos integrar numéricamente la función :math:`\sin(x)` de 0 a :math:`pi`, cuyo valor exacto es 2. Veamos cómo se puede calcular numéricamente con distinto número de muestras::
...
En la prática, la función de uso general más eficiente es :func:`quad(func, a, b)`, que integra por cuadratura de Clenshaw-Curtis [#]_ una función de Python :func:`func` entre ``a`` y ``b``. Consideremos como ejemplo el cálculo del área una semicircunferencia de radio unidad calculando la integral bajo la curva. Para ello definimos una función de circunferencia e integramos entre -1 y 1:
...
La función :func:`quad()` devuelve por defecto el valor de la integral, que en este caso vale :math:`\frac{1}{2}\pi` y una estimación del error en el proceso de integración numérica. Los límites de integración pueden ser :math:`+\infty` o :math:`-\infty` usando los símbolos ``+Inf`` o ``-Inf``::
...
Es posible incluir varios parámetros a la función empleando el parámetro opcional ``args`` como una tupla de parámetros (ver la ayuda de :func:`quad()`::
...
Se pueden calcular integrales dobles empleando de manera similar :func:`dblquad()`, aunque los límites de la segunda integral se deben poner como funciones de Python.::
...
Una maneras de ver *arrays* bidimensionales de :mod:`Numpy` es como matrices, aunque en realidad los *arrays*, cuando se opera algebraicamente con ellos, no se manipulan como matrices. Por ejemplo el producto de dos arrays bidimensionales **NxM** se raliza elemento a elemento y no como un producto algebraico de matrices. Veamos unos ejemplos::
...
Sin embargo :mod:`Numpy` permite hacer el producto punto entre matrices con la función func:`dot()`::
...
Si se va a operar a menudo con matrices es conveniente usar el comando :func:`mat()` de :func:`numpy`, es una abreviatura de ``matrix``. Un elemento ``matrix`` es idéntico a un array y se crea de igual manera o a partir de *arrays*, pero se comporta como una matriz::
...
este cálculo lo haríamos con :mod:`Scipy` de la siguiente manera::
